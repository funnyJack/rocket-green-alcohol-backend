package com.funnyjack.monolith.dsl
...
//https://github.com/consoleau/kotlin-jpa-specification-dsl/compare/fetch_experiment
// Helper to allow joining to Properties
fun <Z, T, R> From<Z, T>.join(
    prop: KProperty1<T, R?>,
    joinType: JoinType = JoinType.INNER
): Join<T, R> = this.join(prop.name, joinType)

fun <Z, T, R> From<Z, T>.leftJoin(prop: KProperty1<T, R?>): Join<T, R> = join(prop, JoinType.LEFT)

fun <Z, T, R> From<Z, T>.joinCollection(
    prop: KProperty1<T, R?>,
    joinType: JoinType = JoinType.INNER
): CollectionJoin<T, R> = this.joinCollection(prop.name, joinType)

fun <Z, T, R> From<Z, T>.joinList(
    prop: KProperty1<T, R?>,
    joinType: JoinType = JoinType.INNER
): ListJoin<T, R> = this.joinList(prop.name, joinType)

fun <Z, T, R> From<Z, T>.joinSet(
    prop: KProperty1<T, R?>,
    joinType: JoinType = JoinType.INNER
): SetJoin<T, R> = this.joinSet(prop.name, joinType)

fun <Z, T, R> From<Z, T>.joinMap(
    prop: KProperty1<T, R?>,
    joinType: JoinType = JoinType.INNER
): MapJoin<Z, T, R> = this.joinMap(prop.name, joinType)

fun <T> Specification<T>.distinct(): Specification<T> = this.and { _, query, _ ->
    query.distinct(true)
    null
}
...
// Empty Specification
inline fun <reified T> emptySpecification(): Specification<T> = Specification.unrestricted()